...
import { InputStream } from "../../lib/spi/InputStream";
...
describe("InputState", () => {
    ...
    function withBuffer(bufSize: number, extraContent: string = "") {
        ...
        assert.strictEqual(is.peek(7) , "0123456");
        is = is.consume("01", "");
        assert.strictEqual(is.peek(3) , "234", "");
        is = is.consume("234567", "");
        assert.strictEqual(is.peek(1) , "8");
    }
    ...
    it("read ahead does not dirty parent", () => {
        ...
        assert.strictEqual(state1.offset , 1);
        const state2 = state1.advance();
        const state3 = state2.advance().advance();
        assert.strictEqual(state3.consume("quick", "").peek(" brown".length) , " brown");
        assert.strictEqual(state0.offset , 0);
    ...
    it("backtracking does not overwrite buffer", () => {
        ...
        for (const s of [easyMatch, requiresBacktracking, requiresMoreBacktracking]) {
            ...
            assert.strictEqual(pm.version , "0.1.1");
        }
    });
